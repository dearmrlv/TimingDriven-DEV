...
OUTPUT_TAG=${OUTPUT_TAG:-"dcf_v7_candidate_expansion_stage1"}
RUN_LABEL=${RUN_LABEL:-""}
DEFAULT_CASES=(superblue16 superblue3 superblue18)
METHOD_KEYS=${METHOD_KEYS:-"efficient_tdp_rerun,dcf_v7_candidate_expansion_e001,dcf_v7_candidate_expansion_e002,dcf_v7_candidate_expansion_e005"}
...
METHOD_SPECS=(
  "efficient_tdp_rerun|Efficient-TDP|pin2pin|-"
  "dcf_v7_candidate_expansion_e001|DCF v7 candidate expansion|dcf_candidate_expansion|0.01"
  "dcf_v7_candidate_expansion_e002|DCF v7 candidate expansion|dcf_candidate_expansion|0.02"
  "dcf_v7_candidate_expansion_e005|DCF v7 candidate expansion|dcf_candidate_expansion|0.05"
)
...
for case_name in "${cases[@]}"; do
  ...
  for spec in "${METHOD_SPECS[@]}"; do
    IFS='|' read -r method_key method_label scheme expansion_fraction <<< "$spec"
    ...
    "$VENV_PYTHON" - "$source_config" "$runtime_config" "$method_result_dir" "$diagnostics_root" "$method_key" "$scheme" "$expansion_fraction" "$RUN_LABEL" "$DCF_HYBRID_DEBUG" "$DCF_HYBRID_DEBUG_DUMP_FIRST_TIMING_STEP_ONLY" "$DCF_HYBRID_DEBUG_DUMP_DIR" "$DCF_HYBRID_DEBUG_SUMMARY_ONLY" "$DCF_DIAG_FIRST_STEP_ONLY" <<'PY'
import json
import pathlib
import sys
...
scheme = sys.argv[6]
expansion_fraction = sys.argv[7]
run_label = sys.argv[8]
hybrid_debug = bool(int(sys.argv[9]))
hybrid_debug_first_step_only = bool(int(sys.argv[10]))
hybrid_debug_dump_dir = sys.argv[11]
hybrid_debug_summary_only = bool(int(sys.argv[12]))
...
data["enable_dcf"] = 1
if expansion_fraction not in {"", "-"}:
    data["dcf_candidate_expansion_fraction"] = float(expansion_fraction)
