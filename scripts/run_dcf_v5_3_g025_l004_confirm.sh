...
OUTPUT_TAG=${OUTPUT_TAG:-"dcf_v5_3_g025_l004_confirm"}
RUN_LABEL=${RUN_LABEL:-""}
DEFAULT_CASES=(superblue16 superblue3 superblue18)
METHOD_KEY=${METHOD_KEY:-"dcf_v5_3_g025_l004_confirm"}
METHOD_LABEL=${METHOD_LABEL:-"DCF v5.3 gated hybrid"}
...
for case_name in "${cases[@]}"; do
  ...
  "$VENV_PYTHON" - "$source_config" "$runtime_config" "$method_result_dir" "$diagnostics_root" "$METHOD_KEY" "$RUN_LABEL" "$DCF_HYBRID_DEBUG" "$DCF_HYBRID_DEBUG_DUMP_FIRST_TIMING_STEP_ONLY" "$DCF_HYBRID_DEBUG_DUMP_DIR" "$DCF_HYBRID_DEBUG_SUMMARY_ONLY" "$DCF_DIAG_FIRST_STEP_ONLY" <<'PY'
import json
import pathlib
import sys
...
hybrid_debug = bool(int(sys.argv[7]))
hybrid_debug_first_step_only = bool(int(sys.argv[8]))
hybrid_debug_dump_dir = sys.argv[9]
hybrid_debug_summary_only = bool(int(sys.argv[10]))
diag_first_step_only = bool(int(sys.argv[11]))
...
data["gpu"] = 1
data["net_weighting_scheme"] = "dcf_hybrid"
data["enable_net_weighting"] = 1
data["pin2pin_net_weighting"] = 1
data["enable_dcf"] = 1
data["dcf_hybrid_lambda"] = 0.04
data["dcf_hybrid_gate_fraction"] = 0.25
