...
METHOD_ORDER = [
    ...
    ("dcf_v5_1_gated_g010", "DCF v5.1 gated hybrid", "0.10", "0.10"),
    ("dcf_v5_1_gated_g025", "DCF v5.1 gated hybrid", "0.10", "0.25"),
    ("dcf_v5_1_gated_g050", "DCF v5.1 gated hybrid", "0.10", "0.50"),
]

GATED_METHODS = METHOD_ORDER[2:]
TRAJECTORY_METHODS = METHOD_ORDER[1:]
...
def main() -> None:
    ...
    parser.add_argument("--tag", default="dcf_v5_1_gated_stage1")
    ...
    for case_name, baseline in BASELINES.items():
        ...
        for method_key, method_name, lam, gate_fraction in METHOD_ORDER[1:]:
    ...
    write_csv(
        artifacts_root / "dcf_v5_1_gated_stage1_metrics.csv",
        metric_rows,
        [
            "case",
            "method",
    ...
    for case_name in BASELINES:
        for method_key, _, _, gate_fraction in GATED_METHODS:
            summary_path = (
    ...
    engagement_rows.sort(
        key=lambda row: (case_order[row["case_name"]], float(row["gate_fraction"]))
    )
    write_csv(
        artifacts_root / "dcf_v5_1_gated_stage1_engagement_sanity.csv",
        engagement_rows,
        [
            "case_name",
            "gate_fraction",
    ...
    for method_key, method_name, _, gate_fraction in TRAJECTORY_METHODS:
        path = (
            result_root
            / "diagnostics"
            / "superblue16"
    ...
    write_csv(
        artifacts_root / "dcf_v5_1_gated_superblue16_trajectory.csv",
        trajectory_rows,
        [
            "timing_step_id",
            "method",
