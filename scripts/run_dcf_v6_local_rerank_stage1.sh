...
OUTPUT_TAG=${OUTPUT_TAG:-"dcf_v6_local_rerank_stage1"}
RUN_LABEL=${RUN_LABEL:-""}
DEFAULT_CASES=(superblue16 superblue3 superblue18)
METHOD_KEYS=${METHOD_KEYS:-"efficient_tdp_rerun,dcf_v6_local_rerank_w005,dcf_v6_local_rerank_w010,dcf_v6_local_rerank_w020"}
...
METHOD_SPECS=(
  ...
  "dcf_v6_local_rerank_w005|DCF v6 local rerank|dcf_local_rerank|0.05|0.20|0.25|0.04"
  "dcf_v6_local_rerank_w010|DCF v6 local rerank|dcf_local_rerank|0.10|0.20|0.25|0.04"
  "dcf_v6_local_rerank_w020|DCF v6 local rerank|dcf_local_rerank|0.20|0.20|0.25|0.04"
)
...
for case_name in "${cases[@]}"; do
  ...
  for spec in "${METHOD_SPECS[@]}"; do
    IFS='|' read -r method_key method_label scheme window_fraction rerank_alpha reference_gate_fraction reference_lambda <<< "$spec"
    ...
    "$VENV_PYTHON" - "$source_config" "$runtime_config" "$method_result_dir" "$diagnostics_root" "$method_key" "$scheme" "$window_fraction" "$rerank_alpha" "$RUN_LABEL" "$DCF_HYBRID_DEBUG" "$DCF_HYBRID_DEBUG_DUMP_FIRST_TIMING_STEP_ONLY" "$DCF_HYBRID_DEBUG_DUMP_DIR" "$DCF_HYBRID_DEBUG_SUMMARY_ONLY" "$DCF_DIAG_FIRST_STEP_ONLY" <<'PY'
import json
import pathlib
import sys
...
scheme = sys.argv[6]
window_fraction = sys.argv[7]
rerank_alpha = sys.argv[8]
...
data["enable_dcf"] = 1
if window_fraction not in {"", "-"}:
    data["dcf_local_rerank_window_fraction"] = float(window_fraction)
if rerank_alpha not in {"", "-"}:
    data["dcf_local_rerank_alpha"] = float(rerank_alpha)
...
PY
    ...
    beta_value=""
    if [ "$scheme" = "dcf_local_rerank" ]; then
      beta_value="0.04"
    fi
