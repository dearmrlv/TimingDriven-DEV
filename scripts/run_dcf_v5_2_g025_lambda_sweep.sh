...
OUTPUT_TAG=${OUTPUT_TAG:-"dcf_v5_2_g025_lambda_sweep"}
RUN_LABEL=${RUN_LABEL:-""}
DEFAULT_CASES=(superblue16 superblue3 superblue18)
METHOD_KEYS=${METHOD_KEYS:-"dcf_v5_1_gated_g025_l010,dcf_v5_2_g025_l004,dcf_v5_2_g025_l006,dcf_v5_2_g025_l008,dcf_v5_2_g025_l010"}
...
METHOD_SPECS=(
  "dcf_v5_1_gated_g025_l010|DCF v5.1 gated hybrid|dcf_hybrid||0.10|0.25"
  "dcf_v5_2_g025_l004|DCF v5.2 gated hybrid|dcf_hybrid||0.04|0.25"
  "dcf_v5_2_g025_l006|DCF v5.2 gated hybrid|dcf_hybrid||0.06|0.25"
  "dcf_v5_2_g025_l008|DCF v5.2 gated hybrid|dcf_hybrid||0.08|0.25"
  "dcf_v5_2_g025_l010|DCF v5.2 gated hybrid|dcf_hybrid||0.10|0.25"
)
...
for case_name in "${cases[@]}"; do
  ...
  for spec in "${METHOD_SPECS[@]}"; do
    IFS='|' read -r method_key method_label scheme version hybrid_lambda gate_fraction <<< "$spec"
    ...
    "$VENV_PYTHON" - "$source_config" "$runtime_config" "$method_result_dir" "$diagnostics_root" "$method_key" "$scheme" "$version" "$hybrid_lambda" "$gate_fraction" "$RUN_LABEL" "$DCF_HYBRID_DEBUG" "$DCF_HYBRID_DEBUG_DUMP_FIRST_TIMING_STEP_ONLY" "$DCF_HYBRID_DEBUG_DUMP_DIR" "$DCF_HYBRID_DEBUG_SUMMARY_ONLY" "$DCF_DIAG_FIRST_STEP_ONLY" <<'PY'
import json
import pathlib
import sys
...
gate_fraction = sys.argv[9]
run_label = sys.argv[10]
hybrid_debug = bool(int(sys.argv[11]))
hybrid_debug_first_step_only = bool(int(sys.argv[12]))
hybrid_debug_dump_dir = sys.argv[13]
hybrid_debug_summary_only = bool(int(sys.argv[14]))
diag_first_step_only = bool(int(sys.argv[15]))
...
if gate_fraction:
    data["dcf_hybrid_gate_fraction"] = float(gate_fraction)
