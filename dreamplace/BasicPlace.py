    ...
    def __init__(self, pos, params, placedb, device):
        ...
        with torch.no_grad():
            ...
            if self.target_density < 1 or params.macro_place_flag:
                self.movable_macro_mask = torch.tensor(
                    placedb.movable_macro_mask, dtype=bool, device=device
                )
            ...
            net_degrees = (
                self.flat_net2pin_start_map[1:] - self.flat_net2pin_start_map[:-1]
            )
            self.net_mask_all = (2 <= net_degrees).to(
                torch.uint8
            )  # all valid nets included
            self.net_mask_ignore_large_degrees = torch.logical_and(
                self.net_mask_all, net_degrees < params.ignore_net_degree
            ).to(torch.uint8)  # nets with large degrees are ignored
            ...
            self.num_pins_in_nodes = torch.tensor(
                num_pins_in_nodes, dtype=self.pos[0].dtype, device=device
            )

            # sum of pin weights for each node.
            sum_pin_weights_in_nodes = np.zeros(placedb.num_nodes)
            self.sum_pin_weights_in_nodes = torch.tensor(
                sum_pin_weights_in_nodes, dtype=self.net_weights.dtype, device="cpu"
            )
...
class BasicPlace(nn.Module):
    ...
    def __init__(self, params, placedb, timer):
        ...
        if (
            params.global_place_flag and params.random_center_init_flag
        ):  # move to center of layout
        ...
        if placedb.num_filler_nodes:  # uniformly distribute filler cells in the layout
            if len(placedb.regions) > 0:
                ...
                self.init_pos[
                    placedb.num_physical_nodes + filler_beg : placedb.num_physical_nodes
                    + filler_end
                ] = np.random.uniform(
                    low=placedb.xl,
                ...
                self.init_pos[
                    placedb.num_nodes + placedb.num_physical_nodes : placedb.num_nodes
                    * 2
                ] = np.random.uniform(
                    low=placedb.yl,
    ...
    def build_timing_op(self, params, placedb, timer=None):
        ...
        return timing.TimingOpt(
            ...
            dcf_v4_base_beta=params.dcf_v4_base_beta,
            dcf_v4_decay_start_step=params.dcf_v4_decay_start_step,
    ...
    def build_legalization(self, params, placedb, data_collections, device):
        ...
        def build_legalization_op(pos):
            ...
            if not legal:
                logging.error(
                    "legality check failed in greedy legalization, "
            ...
            if not legal:
                logging.error(
                    "legality check failed in abacus legalization, "
    ...
    def build_fence_region_legalization(
        ...
        node_size_x = torch.cat(
            [
                ...
                node_size_x[
                    num_movable_nodes + num_terminals : num_movable_nodes
                    + num_terminals
                    + num_terminal_NIs
                ],  ## terminal NIs
                node_size_x[
                    num_nodes - num_filler_nodes + filler_beg : num_nodes
                    - num_filler_nodes
                    + filler_end
                ],  ## fillers
            ],
    ...
    def build_detailed_placement(self, params, placedb, data_collections, device):
        ...
        @param device cpu or cuda
        """
        if global_swap is None or k_reorder is None or independent_set_matching is None:
            raise RuntimeError(
                "Detailed placement operators are unavailable in this build"
            )
