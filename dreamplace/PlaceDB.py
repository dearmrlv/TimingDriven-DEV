...
class PlaceDB(object):
    ...
    def sum_pin_weights(self, weights=None):
        ...
        if weights is None:
            weights = torch.zeros(
                self.num_nodes, dtype=self.net_weights.dtype, device="cpu"
            )
        self.pydb.sum_pin_weights(torch.tensor(self.net_weights), weights)
        return weights
    ...
    def initialize_from_rawdb(self, params):
        ...
        self.net_criticality_deltas = np.array(
            pydb.net_criticality_deltas, dtype=self.dtype
        )
    ...
    def initialize_num_bins(self, params):
        ...
        if params.num_bins_x <= 1 or params.num_bins_y <= 1:
            ...
            while num_bins_1d <= 4096:
                ...
                if (
                    found_num_bins > ideal_num_bins / 4
                    and found_num_bins <= ideal_num_bins
                ):
                    break
                num_bins_1d *= 2
            if y_over_x:
...
""" % (
        ...
        self.total_movable_node_area = float(
            np.sum(
                self.node_size_x[: self.num_movable_nodes]
                * self.node_size_y[: self.num_movable_nodes]
            )
        )
        ...
        self.total_fixed_node_area = float(
            np.sum(
                np.maximum(
                    np.minimum(
                        self.node_x[
                            self.num_movable_nodes : self.num_physical_nodes
                            - self.num_terminal_NIs
                        ]
                        + self.node_size_x[
                            self.num_movable_nodes : self.num_physical_nodes
        ...
        self.movable_macro_mask[: self.num_movable_nodes] = (
            movable_node_area > mean_movable_node_area * 10
        ) & (self.node_size_y[: self.num_movable_nodes] > self.row_height * 2)
        self.movable_macro_pins = np.isin(
            self.pin2node_map,
            np.arange(0, self.num_movable_nodes)[self.movable_macro_mask],
        )

        total_movable_macro_area = movable_node_area[self.movable_macro_mask].sum()
        total_movable_cell_area = (
            self.total_movable_node_area - total_movable_macro_area
        )
        ...
        if len(self.regions) > 0:
            virtual_macro_for_fence_region = [
                fence_region.slice_non_fence_region(
                    ...
                    device="cpu",
                    macro_pos_x=self.node_x[
                        self.num_movable_nodes : self.num_movable_nodes
                        + self.num_terminals
                    ],
                    macro_pos_y=self.node_y[
                        self.num_movable_nodes : self.num_movable_nodes
                        + self.num_terminals
                    ],
        ...
        if params.enable_fillers:
            ...
            if len(self.regions) > 0:
                ...
                if range_lb >= range_ub:  # when there are too few cells, i.e., <= 1
                    ...
                    filler_size_x = np.mean(
                        self.node_size_x[node_size_order[range_lb:range_ub]]
                    )
                filler_size_y = self.row_height
                placeable_area = max(
                ...
                self.total_filler_node_area = max(
                    total_cell_space_area * params.target_density
                    - total_movable_cell_area,
                    0.0,
                )
                ...
                if filler_area == 0:
                    ...
                    self.num_filler_nodes = int(
                        round(self.total_filler_node_area / filler_area)
                    )
                    self.node_size_x = np.concatenate(
                        [
                            self.node_size_x,
