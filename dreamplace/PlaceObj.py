...
try:
    ...
    rudy = None
    pin_utilization = None
    nctugr_binary = None
    adjust_node_area = None
...
class PlaceObj(nn.Module):
    ...
    def __init__(
        ...
        num_bins_x = (
            ...
            and global_place_params["num_bins_x"] > 1
            else placedb.num_bins_x
        )
        num_bins_y = (
            global_place_params["num_bins_y"]
            if "num_bins_y" in global_place_params
            and global_place_params["num_bins_y"] > 1
        ...
        if global_place_params["wirelength"] == "weighted_average":
            ...
            assert 0, (
                "unknown wirelength model %s" % (global_place_params["wirelength"])
            )
        ...
        if params.pin2pin_net_weighting:
    ...
    def obj_fn(self, pos):
        ...
        if self.params.pin2pin_net_weighting:
            ...
            result = torch.add(
                result, self.pin2pin_net_weight, alpha=self.pin2pin_weight
            )
    ...
    def build_pin2pin_net_weight(self, params, placedb, data_collections, pin_pos_op):
    ...
    def build_update_density_weight(self, params, placedb, algo="overflow"):
        ...
        def update_density_weight_op_overflow(cur_metric, prev_metric, iteration):
            assert self.quad_penalty == True, logging.error(
                "density weight update based on overflow only works for quadratic density penalty"
            )
            ### based on overflow
    ...
    def build_noise(self, params, placedb, data_collections):
        ...
        def noise_op(pos, noise_ratio):
            with torch.no_grad():
                ...
                noise[
                    placedb.num_nodes + placedb.num_movable_nodes : 2
                    * placedb.num_nodes
                    - placedb.num_filler_nodes
                ].zero_()
    ...
    def build_route_utilization_map(self, params, placedb, data_collections):
        ...
        """
        if rudy is None:
            raise RuntimeError("Routability operators are unavailable in this build")
    ...
    def build_fence_region_density_op(self, fence_region_list, node2fence_region_map):
        assert type(fence_region_list) == list and len(fence_region_list) == 2, (
    ...
    def build_multi_fence_region_density_op(self):
        ...
        def merged_density_overflow_op(pos):
            ...
            overflow_list, max_density_list = (
                torch.stack(overflow_list),
                torch.stack(max_density_list),
            )
